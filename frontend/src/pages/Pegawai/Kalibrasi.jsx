  ...
  const handleNextClick = async () => {
    if (capturedImage) {
      try {
        ...
        const response = await axios.post('http://127.0.0.1:5000/calibration', postData, {
          headers: {
        ...
        if (check.length > 0 ){
          const updaCalibration = await updateCalibration(check[0].id_calibration,{
    ...
    <div className="flex flex-col h-screen px-16 py-4 overflow-hidden vm:px-5 md:px-14 vm:p-0">
      ...
      <div className="flex items-center min-h-full text-[#3E0000] vm:px-5 border-3 vm:flex-col-reverse vm:justify-end">
        <div className="basis-1/2">
          <h1 className="text-8xl font-extrabold vm:text-7xl vm:hidden">Kalibrasi</h1>
          <p className="text-2xl mt-4 vm:hidden">Kalibrasi akun anda agar lebih optimal</p>
          ...
          <div className="flex gap-5 vm:justify-center">
            <button className="flex items-center gap-3 px-7 py-2 border-2 w-max rounded-2xl border-[#3E0000] bg-[#3E0000] text-white mt-9" onClick={handleNextClick}>
              <span>
                <img src={calibrationIcon} alt="icon calibration" />
              </span>
              Kalibrasi
            </button>
            <button onClick={retake} className="flex items-center gap-3 px-7 py-2 border-2 w-max rounded-2xl border-[#3E0000] bg-[#3E0000] text-white mt-9">
              <span>
                <img src={calibrationIcon} alt="icon calibration" />
              </span>
              Retake
